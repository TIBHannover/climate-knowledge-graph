...
$out += '- Lookup strategies used: OpenAlex API using direct URL endpoints (see data/doi/fetch_openalex.py).'
$out += '- Nearly all IPCC book-chapters and monographs are indexed in OpenAlex.'
$out += ''
$out += '### Copyright and Access Profile'
$out += ''
$licenses = ($csv | Group-Object license | Sort-Object Count -Descending)
$out += 'License | Count'
$out += '--- | ---'
foreach ($l in $licenses) { $out += ($l.Name + ' | ' + $l.Count.ToString()) }
$out += ''
$oaStatus = ($csv | Group-Object oa_status | Sort-Object Count -Descending)
$out += 'Open Access Status | Count'
$out += '--- | ---'
foreach ($o in $oaStatus) { $out += ($o.Name + ' | ' + $o.Count.ToString()) }
$out += ''
...
$out += '- OpenAlex IDs retrieved via data/doi/fetch_openalex.py.'
...
$out += ('- OpenAlex checker: data/doi/fetch_openalex.py')
